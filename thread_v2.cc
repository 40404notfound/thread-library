...
class thread::impl {
    ...
    static const size_t DELETE_STACK_SIZE = 0x100;
    char stackd[DELETE_STACK_SIZE];
...
public:
    ...
    static void thread_delete(thread::impl* p);
    ~impl();
};
...
thread::impl::impl(thread* p, thread_startfunc_t fn, void* arg) {
    ...
    ucd.uc_link = nullptr;
    ucd.uc_stack.ss_sp = &stackd;
    ucd.uc_stack.ss_size = DELETE_STACK_SIZE;
    ucd.uc_stack.ss_flags = SS_DISABLE;
    makecontext(&uc, reinterpret_cast<void(*)()>(thread::impl::thread_start), 2, fn, arg);
    makecontext(&ucd, reinterpret_cast<void(*)()>(thread::impl::thread_delete), 1, this);
    if (fn == idle_func) {
...
void thread::impl::thread_start(void(*fn)(void*), void* arg) {
    ...
    // this is the problem
    // it may not return so the context never ends
    cpu::impl::run_next();
}

void thread::impl::thread_delete(thread::impl* p) {
    delete p;
}
